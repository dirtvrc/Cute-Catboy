...
class Fun_Commands(commands.Cog):
    ...
    @commands.command()
    async def karin(self, ctx):
        """ Karin is gay """
        bio = "https://c.tenor.com/e8s1VZmddzwAAAAM/cry-bozo.gif"
        await ctx.send("Karin is gay, Rusk on top bozo!")
        await ctx.send(bio)

    @commands.command()
    async def rusk(self, ctx):
        """ Rusk on top """
        bio = "https://c.tenor.com/epNMHGvRyHcAAAAC/gigachad-chad.gif"
        await ctx.send("Rusk Best Boy!")
        await ctx.send(bio)
        await ctx.send("Average Rusk User ^")
